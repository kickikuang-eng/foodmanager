...
async function fetchYouTubeAuthor(videoUrl) {
  try {
...
async function analyzeVideoForRecipe(imageUrl, videoUrl) {
  ...
  const prompt = isActualImage ? `Analyze this image from a cooking video and extract a complete recipe. Please provide:
...
Be creative and detailed. If some information isn't visible, make reasonable assumptions based on what you can see.` : `I cannot access the actual thumbnail from this ${videoUrl.includes('instagram') ? 'Instagram' : 'TikTok'} video due to platform restrictions. 
...
function placeholderRecipe(videoUrl) {
  ...
  return {
    ...
    ingredients: [
      '1 cup placeholder ingredient',
      '2 teaspoons placeholder spice',
      '1 tablespoon placeholder oil',
      'Salt to taste',
      'Pepper to taste'
    ],
    instructions: [
