...
function getVideoThumbnail(info) {
...
async function analyzeWithGemini(params) {
  ...
  const parts: any[] = [
    {
      text: buildPrompt(params.videoUrl)
    }
  ];
...
function normalizeRecipe(raw) {
  const recipe = {
    ...
    ingredients: Array.isArray(raw?.ingredients) ? raw.ingredients.map((i)=>String(i)).slice(0, 50) : [],
    instructions: Array.isArray(raw?.instructions) ? raw.instructions.map((s)=>String(s)).slice(0, 100) : [],
